...
import {Spin, Tag, Menu, Icon, Avatar, Tooltip, Switch, Modal} from 'antd';
...
export default class GlobalHeaderRight extends PureComponent {
  ...
  componentDidMount() {
    console.log(sessionStorage.getItem('printSwitch'))
    let autoPrint = sessionStorage.getItem('printSwitch') || false;
    this.setState({
      autoPrint: autoPrint === '1' ? true : false
    })
  }
  ...
  handleOpenPrint = (check) => {
    let _that = this;
    if (check) {
      Modal.confirm({
        title: '是否开启自动打印',
        content: '请确认是否已连接上打印设备？(无连接设备将使自动打印失效！)',
        okText: '确定',
        okType: 'danger',
        cancelText: '取消',
        onOk() {
          ...
          sessionStorage.setItem('printSwitch', '1')
          getNewOrder();
        },
        onCancel() {
          _that.setState({
          ...
          sessionStorage.setItem('printSwitch', '0')
        },
      });
  ...
  render() {
    ...
    return (
      <div className={className} style={{ marginRight: 8 }}>
        ...
        {/*</NoticeIcon>*/}
        {
          groupCode === 'gasstation' ? (
            <Fragment>
              <span>是否开启自动打印：</span>
              <Switch checked={this.state.autoPrint} onChange={this.handleOpenPrint} />
            </Fragment>
          ) : ''
        }
        {currentUser.name ? (
